...
if ($Date_DB_nov->format('Ymdhis') > $Date_Local_nov->format('Ymdhis')):  // Si la fecha y hora de las novedades en la base de datos es mayor que la del archivo local
    ...
    file_put_contents( __DIR__ . "/logs/data/fechaHoraNovedades.json" , json_encode(lastUpdateTabla($link, 'NOVEDAD'),JSON_PRETTY_PRINT), LOCK_EX);  // actualizamos fecha hora de novedades
    fileLogs("Se actualizo el archivo \"fechaHoraNovedades.json\"", __DIR__ . "/logs/info/" . date('Ymd') . "_informacion.log", '');
    
    file_put_contents( __DIR__ . "/logs/data/novedades.json" , json_encode($objetoNovedadesCH, JSON_PRETTY_PRINT), LOCK_EX); // Actualizamos el archivo local con el objeto de las novedades
